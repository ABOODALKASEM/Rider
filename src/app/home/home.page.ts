...
async handleRideUpdates(doc) {
  ...
  switch (data.status) {
    ...
    case 'started':
      ...
      break;
    case 'completed':  // Changed from 'stopped' to 'completed'
...
async handleRideStop(doc) {
  console.log('Ride completed, entering rating stage');
  ...
  // Ensure we're exiting the driving to destination stage
  this.drivingToDestinationStage = false;
  
  // Enter the rating stage
  this.EnterRatingStage();
...
EnterRatingStage() {
  console.log('Entering rating stage');
  this.setStage('ratingStage');
  this.setMapHeight(); // Adjust map height for rating stage
}
  ...
  try {
    ...
    for (const marker of markersToRemove) {
      if (marker) {
        try {
          ...
          console.error(`Error removing marker: ${marker}`, error);
        }
      }
    }
    ...
    if (this._carmarkers && this._carmarkers.length > 0) {
      for (const { marker } of this._carmarkers) {
        try {
          await this.map.newMap.removeMarker(marker);
          console.log(`Car marker removed: ${marker}`);
        } catch (error) {
          console.error(`Error removing car marker: ${marker}`, error);
        }
      }
      this._carmarkers = [];
    }
...
async ReturnHome() {
  console.log('Returning to home stage');
  await this.ResetState();
  this.EnterBookingStage();
  this.stopPolling = false;
  await this.map.newMap.setCamera({
...
private updateStage(stages: { [key: string]: boolean }) {
  ...
  this.ratingStage = stages.ratingStage;
}
...
async submitRating() {
  console.log('Submitting rating:', this.rating, 'Comment:', this.comment);
  try {
    // Prepare the rating data
    const ratingData = {
      rating: this.rating,
      comment: this.comment || '', // Use an empty string if comment is undefined
      driverId: this.driverInfo.Driver_ID,
    ...
    // Submit the rating
    await this.database.submitRating(ratingData);

    this.overlay.showAlert('Thank You', 'Your rating has been submitted successfully!');
    await this.ReturnHome();
  } catch (error) {
    console.error('Error submitting rating:', error);
    this.overlay.showAlert('Error', 'Failed to submit rating. Please try again.');
  }
}
